...
address='/Users/mac/Documents/Stage_EPFL/MatLab/';
%filename='ASSBLY_CASEA_2L.txt';
%outdata='CASEA_2L';
filename='ASSBLY_CASEA_2A_1L_corr.txt';
outdata='2A_1L';
%define ref
%refFile = readtable([address 'refPinPowerMaps.csv']);
refFile =  csvread([address 'refPinPowerMaps.csv'],1,0);
lineNum = double(filename(15))-64;
ref = refFile(lineNum,:);
nmix = 161;
ngrpd = 2;
nbnus = 4;
...
tot_pow=sum(nftot_d_1g_vol)/max(merge);
for i=1:max(merge)
    pow(i)=sum(nftot_d_1g_vol(find(merge==i)))./tot_pow;
end
